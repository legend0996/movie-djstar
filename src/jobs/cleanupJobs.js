...
function startScheduledJobs() {
  cron.schedule(config.jobs.cleanupSchedule, async () => {
    ...
    try {
      ...
      logger.error('Failed to clean old activity logs', { error: err.message });
    }

    try {
      await db.execute(
        `DELETE FROM stream_log WHERE started_at < DATE_SUB(NOW(), INTERVAL 90 DAY)`
      );
    } catch (err) {
      logger.error('Failed to clean old stream logs', { error: err.message });
    }
