...
const paymentService = {
  ...
  async handleCallback(callbackBody, signature) {
    const callback = mpesaService.verifyCallback(callbackBody);
    const signatureValid = mpesaService.verifyCallbackSignature(callbackBody, signature);

    if (!callback.valid || !signatureValid) {
      logger.warn('Invalid M-Pesa callback received', {
        reason: signatureValid ? 'Invalid structure' : 'Invalid signature',
      });
      return { valid: false };
    }
    ...
    logger.info('M-Pesa callback received', {
      ...
      mpesaReceipt: callback.mpesaReceipt ? '[REDACTED]' : null,
    });
