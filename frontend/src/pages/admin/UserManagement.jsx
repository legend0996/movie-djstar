...
export default function UserManagement() {
  ...
  const { data, isLoading, refetch } = useFetch(
    ['admin-users', page, search, statusFilter, roleFilter],
    `/admin/users?page=${page}&limit=20${search ? `&q=${search}` : ''}${statusFilter ? `&status=${statusFilter}` : ''}${roleFilter ? `&role=${roleFilter}` : ''}`
  );
