...
const adminService = {
  ...
  async getDeveloperDashboard() {
    ...
    const activeUsers = await userRepository.countByStatus(USER_STATUS.ACTIVE);
    const unverifiedUsers = await userRepository.countByStatus(USER_STATUS.UNVERIFIED);
    const suspendedUsers = await userRepository.countByStatus(USER_STATUS.SUSPENDED);
  ...
  async updateUserStatus(userId, status) {
    ...
    if (status === USER_STATUS.SUSPENDED || status === USER_STATUS.DISABLED) {
      await db.execute(
  ...
  async getRevenueReports({ startDate, endDate } = {}) {
    const end = endDate ? new Date(endDate) : new Date();
    const start = startDate ? new Date(startDate) : new Date(end.getTime() - 30 * 86400000);
